...
class CustomerStatementPortal(CustomerPortal):
    ...
    def portal_my_customer_statements(self, **kw):
        ...
        partner_id.with_context(compute_statement=True)._compute_customer_statements()
    ...
    def customer_statement_xls(self, **post):
        res = request.env.user.partner_id.with_context(always_show_bucket=True).action_print_customer_statement_xls()
    ...
    def filter_customer_statement_get(self, **post):
        ...
        if post.get("start_date") and post.get("end_date"):
            ...
            if statement_lines:
                ...
                all_lines = request.env.user.partner_id.sh_filter_customer_statement_ids.sorted(
                    key=lambda l: l.sh_filter_invoice_date or datetime.date.today()
                )
                running_total = 0.0
