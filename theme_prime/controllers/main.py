...
class ThemePrimeWebsiteSale(WebsiteSale):
    ...
    def _get_categories_list(self, **post):
        ...
        categories = request.env['product.public.category'].search(website.website_domain())
        categories_data = []
        for cat in categories:
            categories_data.append({
                'id': cat.id,
                'name': cat.name,
                'parent_id': cat.parent_id.id or False,
                'child_id': cat.child_id.ids,
                'img_small': website.image_url(cat, 'image_128'),
                'dr_category_sidebar_cover': website.image_url(cat, 'dr_category_sidebar_cover'),
            })
        return {
            'categories': categories_data,
            'category_count': request.env['product.template']._get_product_category_count(domain=website.website_domain()) if website._get_dr_theme_config('json_sidebar_config').get('category_sidebar_show_count') else {},
        }
...
class ThemePrimeMainClass(http.Controller):
    ...
    def get_dialog_content(self, res_id, fields, **post):
        return request.env['dr.website.content'].search_read([('id', '=', res_id)], fields=fields)
    ...
    def get_categories_info(self, fields=[], options={}, **kwargs):
        ...
        for res_category, category in zip(result, categories):
            ...
            res_category['image_url'] = website.image_url(category, 'image_512')
            res_category['cover_image'] = website.image_url(category, 'dr_category_cover_image')
        return result
    ...
    def get_megamenu_categories(self, options={}, limit=5, fields=['name', 'id'], order='count', **kwargs):
        ...
        for category in categories:
            all_categories.append({
                ...
                'image_url': website.image_url(category, 'image_512'),
                'cover_image': website.image_url(category, 'dr_category_cover_image'),
                'dr_category_icon': website.image_url(category, 'dr_category_icon'),
...
class ThemeWebsite(Website):
    ...
    def dr_search(self, term, max_nb_chars, options, **kw):
        ...
        search_order = options.get('order') if options.get('order', False) else 'name asc'
        ...
        if fuzzy_enabled:
            search_details = request.website._search_get_details('products', search_order, {**options, 'allowFuzzy': True})
        ...
        if product_limit:
            results['products'] = self.autocomplete(search_type='products_only', term=term, order=search_order, limit=product_limit, options=options)
        ...
        def _match_categories(trm, p_limit, match_any_word=None):
            ...
            return {'results': [{'id': categ[0].id, 'name': self.format_result(categ[1], categ[0].ds_name), 'image_url': categ[0].image_url} for categ in category_result], 'results_count': len(category_result), 'parts': {}}, p_limit
