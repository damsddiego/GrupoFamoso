...
class InheritProductTemplate(models.Model):
    ...
    def create(self, vals_list):
        products = super(InheritProductTemplate, self).create(vals_list)
        print("products------------->", products)
        return products

    @api.model
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        res = super(InheritProductTemplate, self).read_group(domain, fields, groupby, offset=offset, limit=limit,
...
class gen_sale(models.TransientModel):
    ...
    def create_product(self, values):
        ...
        try:
            if float(values.get('on_hand')):
                if float(values.get('on_hand')) >0:
                    categ_type = 'consu'
            ...
            if count > 25:
                ...
                if normal_details.startswith('x_'):
                    ...
                    if any_special:
                        ...
                        many2x_fields1 = self.env['ir.model.fields'].search(
                            [('name', '=', technical_fields_name), ('state', '=', 'manual'),
                             ('model_id', '=', model_id1.id)])
                        many2x_fields2 = self.env['ir.model.fields'].search(
                            [('name', '=', technical_fields_name), ('state', '=', 'manual'),
                             ('model_id', '=', model_id2.id)])
                        if many2x_fields1.id:
                            if many2x_fields1.ttype in ['many2one', 'many2many']:
                                if many2x_fields1.ttype == "many2one":
                                    if values.get(i):
                                        fetch_m2o = self.env[many2x_fields1.relation].search(
                                            [('name', '=', values.get(i))])
                                        if fetch_m2o.id:
                                            custom_vals.update({
                                ...
                                if many2x_fields1.ttype == "many2many":
                                    ...
                                    if values.get(i):
                                        if ';' in values.get(i):
                                            ...
                                            m2m_names = values.get(i).split(',')
                                            m2m_id = self.env[many2x_fields1.relation].search(
                                                [('name', 'in', m2m_names)])
                                            if not m2m_id:
                                                raise ValidationError(
                                                    _('"%s" This custom field value "%s" not available in system') % (
                        ...
                        if many2x_fields2.id:
                            if many2x_fields2.ttype in ['many2one', 'many2many']:
                                ...
                                if many2x_fields2.ttype == "many2one":
                                    if values.get(i):
                                        fetch_m2o = self.env[many2x_fields2.relation].search(
                                            [('name', '=', values.get(i))])
                                        if fetch_m2o.id:
                                            custom_vals.update({
                                                technical_fields_name: fetch_m2o.id
                                ...
                                if many2x_fields2.ttype == "many2many":
                                    ...
                                    if values.get(i):
                                        if ';' in values.get(i):
                                            ...
                                            m2m_names = values.get(i).split(',')
                                            m2m_id = self.env[many2x_fields2.relation].search(
                                                [('name', 'in', m2m_names)])
                                            if not m2m_id:
                                                raise ValidationError(
                                                    _('"%s" This custom field value "%s" not available in system') % (
                        ...
                        normal_fields1 = self.env['ir.model.fields'].search(
                            [('name', '=', normal_details), ('state', '=', 'manual'), ('model_id', '=', model_id1.id)])
                        normal_fields2 = self.env['ir.model.fields'].search(
                            [('name', '=', normal_details), ('state', '=', 'manual'), ('model_id', '=', model_id2.id)])
        ...
        if self.product_option == 'update':
            if self.product_search == 'by_name':
                product_temp = product_tmpl_obj.search(
                    [('name', '=', values.get('name')), ('uniq_id', '=', values.get('u_id'))], limit=1)
            elif self.product_search == 'by_barcode':
                product_temp = product_tmpl_obj.search(
                    [('barcode', '=', values.get('barcode')), ('uniq_id', '=', values.get('u_id'))], limit=1)
            elif self.product_search == 'by_code':
                product_temp = product_tmpl_obj.search(
                    [('default_code', '=', values.get('default_code')), ('uniq_id', '=', values.get('u_id'))], limit=1)
        else:
            product_temp = product_tmpl_obj.search(
                [('name', '=', values.get('name')), ('uniq_id', '=', values.get('u_id'))], limit=1)
        ...
        if product_temp and values.get('attributes'):
            ...
            for pair in atr:
                ...
                attribute_value = self.env['product.attribute.value'].search(
                    [('name', '=', temp[0]), ('attribute_id', '=', pair)], limit=1)
        ...
        if self.product_option == 'update':
            if self.product_search == 'by_name':
                product_temp_id = product_tmpl_obj.search(
                    [('name', '=', values.get('name')), ('uniq_id', '=', values.get('u_id'))], limit=1)
            elif self.product_search == 'by_barcode':
                product_temp_id = product_tmpl_obj.search(
                    [('barcode', '=', values.get('barcode')), ('uniq_id', '=', values.get('u_id'))], limit=1)
            ...
            if values.get('attributes'):
                ...
                for pair in atr:
                    ...
                    product_template_attribute_values = self.env['product.template.attribute.value'].search(
                        [('product_tmpl_id', '=', product_temp_id.id), ('attribute_id', '=', attribute.id),
                         ('name', '=', value_rec.name)])
        ...
        if res:
            if res.type == 'consu':
                ...
                location = self.env['stock.warehouse'].search([('company_id', '=', company_user.id)],
                    ...
                    if self.product_option == 'create':
                        ...
                        if product_ids:
                            ...
                            if is_storable != False:
                                product_ids.write({'is_storable': is_storable or False})
                            if self.product_search == 'by_name':
                                if line[2]:
