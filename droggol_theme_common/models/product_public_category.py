...
class ProductPublicCategory(models.Model):
    ...
    def _get_category_search_tuples(self, lang, website_id):
        ...
        for category in categories:
            ...
            image_url = website.image_url(category, 'image_128')
            if has_formulate:
                cartesian_list = []
                ...
                for cp in itertools.product(*cartesian_list):
                    _add_entry(' '.join(cp), category.id, category.dr_search_formulate, image_url)
            else:
                _add_entry(category.name, category.id, category.dr_search_formulate, image_url)
                for synonym in category.ds_synonyms_ids:
                    _add_entry(synonym.name, synonym.category_id.id, category.dr_search_formulate, image_url)
